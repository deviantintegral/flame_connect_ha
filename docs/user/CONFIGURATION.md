...
This document describes all configuration options and settings available in the FlameConnect custom integration.
...
```yaml
service: flameconnect.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: flameconnect.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/deviantintegral/flameconnect_ha/issues) - Report problems

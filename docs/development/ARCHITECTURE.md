...
This document describes the technical architecture of the FlameConnect custom component for Home Assistant.
...
│   ├── __init__.py          # Exports DeviantintegralDataUpdateCoordinator
...
│   ├── __init__.py          # Exports DeviantintegralEntity
...
- `base.py` - Main coordinator class (`DeviantintegralDataUpdateCoordinator`)
...
**Key class:** `DeviantintegralDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `DeviantintegralApiClient`
...
- `DeviantintegralConfigFlowHandler` (main flow)
- `DeviantintegralOptionsFlow` (options)
...
**Key class:** `DeviantintegralEntity` (in `entity/base.py`)
...
2. `DeviantintegralEntity` for common functionality
...
1. Create directory: `custom_components/flameconnect/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `DeviantintegralEntity`

...
- Integration logs: `custom_components.flameconnect: debug` in `config/configuration.yaml`
...
- Use the domain `flameconnect` for all DOMAIN references
- Prefix all integration-specific classes with `Deviantintegral`
- Use "FlameConnect" as the display title
...
- Inherit from platform base + `DeviantintegralEntity`
...
- `tests/` mirrors `custom_components/flameconnect/` structure

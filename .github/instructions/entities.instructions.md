...
- **`entity/`** - Base entity classes (inherit `FlameConnectEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, FlameConnectEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import FlameConnectEntity`

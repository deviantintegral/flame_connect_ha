...
class FlameConnectRefreshButton(ButtonEntity, FlameConnectEntity):
    ...
    async def async_press(self) -> None:
        """Refresh coordinator data, surviving a cancelled caller.

        ``button.press`` is awaited inside whatever task issued it, and
        this is the recovery control, so it must not be the first thing a
        cancellation throws away; see
        :meth:`FlameConnectDataUpdateCoordinator.async_refresh_shielded`.
        """

...
class FlameConnectApiClientError(Exception):
    """Base exception to indicate a general API error."""


class FlameConnectApiClientCommunicationError(
    FlameConnectApiClientError,
):
    """Exception to indicate a communication error with the API."""


class FlameConnectApiClientAuthenticationError(
    FlameConnectApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        FlameConnectApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise FlameConnectApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class FlameConnectApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            FlameConnectApiClientAuthenticationError: If authentication fails.
            FlameConnectApiClientCommunicationError: If communication fails.
            FlameConnectApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise FlameConnectApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise FlameConnectApiClientError(
                msg,
            ) from exception
